...
begin
	...
	p_rxd_s: process (clk) is -- Avoid detecting Line Glitches and noise
	begin
		if rising_edge(clk) then
			rxd_s0 <= rxd;
			rxd_s1 <= rxd_s0;
	...
	end process p_rxdp_s;

	p_rxdn_s: process (clk) is
	begin
		if rising_edge(clk) then
			rxdn_s0 <= rxdn;
			rxdn_s1 <= rxdn_s0;
	...
	-- DPLL FSM
	p_dpll_state: process (clk, rst) is
	begin
		if rst = '0' then
			dpll_state <= "01";
		elsif rising_edge(clk) then
	...
	end process p_dpll_state;

	p_dpll_next_state: process (dpll_state, lock_en, change) is
	begin
		case (dpll_state) is
			when "00" =>
	...
	end process p_fs_state;

	p_fs_next_state: process (fs_state, fs_ce, k, j, rx_en, rx_active, se0,
	se0_s) is
	begin
	...
	end process p_fs_next_state;

	synced_d <=
		fs_ce and rx_en when (fs_state = K3 and k = '1') or (fs_state = K4 and k = '1') else
		'0';
	...
	p_rx_active: process (clk, rst) is
	begin
		if rst = '0' then
			rx_active <= '0';
		elsif rising_edge(clk) then
	...
	end process p_rx_active;

	p_rx_valid_r: process (clk, rst) is
	begin
		if rst = '0' then
			rx_valid_r <= '0';
		elsif rising_edge(clk) then
	...
	end process p_rx_valid_r;

	--====================================================================================--
	-- NRZI Decoder                                                                       --
	--====================================================================================--

	p_sd_r: process (clk, rst) is
	begin
		if rst = '0' then
			sd_r <= '0';
		elsif rising_edge(clk) then
	...
	p_bit_stuff_err: process (clk, rst) is -- Bit Stuff Error
	begin
		if rst = '0' then
			bit_stuff_err <= '0';
		elsif rising_edge(clk) then
	...
	end process p_bit_stuff_err;

	--====================================================================================--
	-- Serial => Parallel converter                                                       --
	--====================================================================================--

	p_shift_en: process (clk, rst) is
	begin
		if rst = '0' then
			shift_en <= '0';
		elsif rising_edge(clk) then
	...
	p_rx_valid1: process (clk, rst) is
	begin
		if rst = '0' then
			rx_valid1 <= '0';
		elsif rising_edge(clk) then
	...
	end process p_rx_valid1;

	p_rx_valid: process (clk, rst) is
	begin
		if rst = '0' then
			rx_valid <= '0';
		elsif rising_edge(clk) then
	...
	end process p_rx_valid;

	p_se0_r: process (clk) is
	begin
		if rising_edge(clk) then
			se0_r <= se0;
		end if;
